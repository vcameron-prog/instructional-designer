...
Please generate an IMPROVED version that incorporates the requested changes while maintaining quality and comprehensiveness.`;
  ...
  // ============================================================
  // ACCESSIBILITY TOOLKIT TOOLS
  // ============================================================

  // -- Color Contrast -------------------------------------------------------
  // Pure WCAG math — no AI, no auth required.
  ...
  // -- URL Scanner ----------------------------------------------------------
  // Fetches a public URL and runs WCAG AI analysis. No auth required.
...
Limit to the 10 most impactful issues. Be precise and technical.`;
  ...
  app.post(
    "/api/tools/alt-text",
    (req, res, next) =>
      altTextUpload.single("image")(req, res, (err) => {
...
Respond with ONLY the alt text — no explanation, no quotes.`,
  ...
  app.post(
    "/api/tools/math-ocr",
    (req, res, next) =>
      mathOcrUpload.single("image")(req, res, (err) => {
        if (err) return res.status(400).json({ error: err.message });
        next();
      }),
    async (req: Request, res: Response) => {
