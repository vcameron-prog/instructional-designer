...
describe("pruneOldVersions is called correctly from route handlers", () => {
  ...
  describe("POST /api/content/:id/preview-fix", () => {
    ...
    // fix-html-table-caption
    // -----------------------------------------------------------------------
    it("returns { before, after } and calls fixHtmlTableCaption for fix-html-table-caption", async () => {
      const contentId = 730;
      const originalContent = "<table><tr><td>X</td></tr></table>";
      const fixedContent = "<table><caption>Table summary</caption><tr><td>X</td></tr></table>";
      ...
      expect(mockFixHtmlTableCaption).toHaveBeenCalledTimes(1);
      expect(mockFixHtmlTableCaption).toHaveBeenCalledWith(originalContent);
      expect(response.body).toEqual({ before: originalContent, after: fixedContent });
      expect(mockCreateVersion).not.toHaveBeenCalled();
      expect(mockUpdateContent).not.toHaveBeenCalled();
    });

    it("returns identical before/after when fix-html-table-caption makes no change", async () => {
      const contentId = 731;
      const unchangedContent = "<table><caption>Already captioned</caption><tr><td>X</td></tr></table>";
      ...
      // Default mock is a passthrough with tablesFixed: 0
    ...
    // edit-html-table-caption
    // -----------------------------------------------------------------------
    it("returns { before, after } and calls editHtmlTableCaption for edit-html-table-caption", async () => {
      const contentId = 740;
      const originalContent = "<table><caption>Old caption</caption><tr><td>X</td></tr></table>";
      const fixedContent = "<table><caption>New caption</caption><tr><td>X</td></tr></table>";
      ...
      mockEditHtmlTableCaption.mockReturnValue(fixedContent);

      const response = await request(app)
        .post(`/api/content/${contentId}/preview-fix`)
        .send({ fixType: "edit-html-table-caption", captionText: "New caption" })
        .expect(200);

      expect(mockEditHtmlTableCaption).toHaveBeenCalledTimes(1);
      expect(mockEditHtmlTableCaption).toHaveBeenCalledWith(originalContent, "New caption", undefined);
      expect(response.body).toEqual({ before: originalContent, after: fixedContent });
      expect(mockCreateVersion).not.toHaveBeenCalled();
      expect(mockUpdateContent).not.toHaveBeenCalled();
    });
    ...
    // fix-html-table-thead
    // -----------------------------------------------------------------------
    it("returns { before, after } and calls fixHtmlTableThead for fix-html-table-thead", async () => {
      const contentId = 750;
      const originalContent = "<table><tr><td>Header</td></tr><tr><td>Data</td></tr></table>";
      const fixedContent = "<table><thead><tr><th scope=\"col\">Header</th></tr></thead><tr><td>Data</td></tr></table>";
      ...
      expect(mockFixHtmlTableThead).toHaveBeenCalledTimes(1);
      expect(mockFixHtmlTableThead).toHaveBeenCalledWith(originalContent);
      expect(response.body).toEqual({ before: originalContent, after: fixedContent });
      expect(mockCreateVersion).not.toHaveBeenCalled();
      expect(mockUpdateContent).not.toHaveBeenCalled();
    });

    it("returns identical before/after when fix-html-table-thead makes no change", async () => {
      const contentId = 751;
      const unchangedContent = "<table><thead><tr><th scope=\"col\">Header</th></tr></thead><tr><td>Data</td></tr></table>";
