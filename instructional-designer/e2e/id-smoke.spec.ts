...
test.describe("Course creation — form submits and lands on tools page", () => {
  ...
  test("invalid course submission associates the error and focuses the first field", async ({ page }) => {
    await page.goto(appPath("/new-course"));

    const courseName = page.getByTestId("input-course-name");
    await page.getByTestId("button-submit").click();

    await expect(courseName).toBeFocused();
    await expect(courseName).toHaveAttribute("aria-invalid", "true");
    const describedBy = await courseName.getAttribute("aria-describedby");
    expect(describedBy).toBeTruthy();
    const errorId = describedBy!.split(/\s+/).at(-1)!;
    const errorMessage = page.locator(`[id="${errorId}"]`);
    await expect(errorMessage).toHaveText("Course name is required");

    await courseName.fill("Accessible Course");
    await expect(courseName).toHaveAttribute("aria-invalid", "false");
...
test.describe("Quick-tool — assignment form generates content", () => {
  ...
  test("required field errors focus the first custom control and clear when corrected", async ({ page }) => {
    await page.goto(appPath("/quick-tools/assignment"));

    const assignmentType = page.getByTestId("select-assignmentType");
    await page.getByTestId("button-generate").click();

    await expect(assignmentType).toBeFocused();
    await expect(assignmentType).toHaveAttribute("aria-invalid", "true");
    await expect(assignmentType).toHaveAttribute("aria-describedby", "assignmentType-error");
    await expect(page.locator("#assignmentType-error")).toHaveText("Assignment Type is required");

    await assignmentType.click();
    await page.getByRole("option", { name: "Essay/Paper" }).click();
    await expect(assignmentType).toHaveAttribute("aria-invalid", "false");
    await expect(page.locator("#assignmentType-error")).toHaveText("");
  });
  ...
  test("fills assignment form, mocks generation, and result page renders content", async ({
    ...
    const mockContent = {
      ...
      formData: {},
      content:
        "## Overview\n\nThis is a smoke-test assignment generated by the E2E suite.\n\n<table><tr><th>Week</th><th>Topic</th></tr><tr><td>1</td><td>Foundations</td></tr></table>\n\n<table><tr><th>Task</th><th>Due</th></tr><tr><td>Essay</td><td>Friday</td></tr></table>",
      isApproved: false,
    ...
    await page.getByTestId("button-refine").click();
    const refinement = page.getByTestId("textarea-refinement");
    await page.getByTestId("button-submit-refine").click();
    await expect(refinement).toBeFocused();
    await expect(refinement).toHaveAttribute("aria-invalid", "true");
    await expect(refinement).toHaveAttribute("aria-describedby", "refinement-request-error");
    await expect(page.locator("#refinement-request-error")).toHaveText("Describe the changes you'd like to make");
    await refinement.fill("   ");
    await expect(refinement).toHaveAttribute("aria-invalid", "true");
    await refinement.fill("Use clearer headings");
    await expect(refinement).toHaveAttribute("aria-invalid", "false");
    await expect(page.locator("#refinement-request-error")).toHaveText("");
    await page.keyboard.press("Escape");

    await page.getByText("Accessibility Check", { exact: true }).click();
    await page.getByTestId("button-fix-fix-html-table-caption").click();
    const firstCaption = page.getByTestId("input-caption-text-0");
    const secondCaption = page.getByTestId("input-caption-text-1");
    await firstCaption.fill("Weekly schedule");
    await secondCaption.fill("Weekly schedule");
    await page.getByTestId("button-apply-caption").focus();
    await page.getByTestId("button-apply-caption").click();
    await expect(firstCaption).toBeFocused();
    await expect(firstCaption).toHaveAttribute("aria-invalid", "true");
    await expect(firstCaption).toHaveAttribute("aria-describedby", "caption-input-0-error");
    await expect(page.locator("#caption-input-0-error")).toHaveText("Each table must have a unique caption");
    await secondCaption.fill("Assignment deadlines");
    await expect(secondCaption).toHaveAttribute("aria-invalid", "false");
    await page.getByTestId("button-cancel-caption").click();

    await page.getByTestId("button-push-github").click();
    const githubRepo = page.getByTestId("input-github-repo");
    const githubPath = page.getByTestId("input-github-path");
    await page.getByTestId("button-confirm-push-github").click();
    await expect(githubRepo).toBeFocused();
    await expect(githubRepo).toHaveAttribute("aria-invalid", "true");
    await expect(githubRepo).toHaveAttribute("aria-describedby", "github-repo-error");
    await githubRepo.fill("   ");
    await expect(githubRepo).toHaveAttribute("aria-invalid", "true");
    await githubRepo.fill("example/course-materials");
    await expect(githubRepo).toHaveAttribute("aria-invalid", "false");
    await page.getByTestId("button-confirm-push-github").click();
    await expect(githubPath).toBeFocused();
    await expect(githubPath).toHaveAttribute("aria-invalid", "true");
  });
});
