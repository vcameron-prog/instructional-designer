...
- **Metrics API (root app)**: `GET /api/metrics` returns in-memory runtime counters. Currently exposes `aiFixRetry.count` (how many times `fixComplianceIssue` fell back to the strict-prompt retry path since last server restart) and `aiFixRetry.lastAt` (ISO timestamp of the most recent retry). Also exposes `sessionSaveFail.count` (how many times `session.save()` failed after a token refresh since last server restart) and `sessionSaveFail.lastAt` (ISO timestamp of the most recent failure); a non-zero or rising count indicates the PostgreSQL session store is degraded and users are receiving in-memory-only sessions. No auth required. All counters reset on server restart.
- **Metrics API (instructional-designer app)**: `GET /api/metrics` on the instructional-designer sub-app returns runtime counters with threshold-based status fields. Exposes `altTextParseFail.count`, `altTextParseFail.lastAt`, and `altTextParseFail.status` (`"ok"` / `"warn"` / `"critical"`). Also exposes `rateLimitCleanup.lastRunAt`, `rateLimitCleanup.lastErrorAt`, `rateLimitCleanup.rowsDeletedTotal`, and `rateLimitCleanup.status` (`"ok"` / `"warn"`). A `thresholds` object echoes the active threshold values. No auth required. Threshold env vars: `ALT_TEXT_PARSE_FAIL_WARN_COUNT` (default 5) and `ALT_TEXT_PARSE_FAIL_CRITICAL_COUNT` (default 15). **Interpreting a rising `altTextParseFail.count`**: each increment means Claude's alt-text response failed to parse as the expected `{altText, confidence}` JSON shape even after a retry, causing the confidence field to be silently dropped; a `"warn"` status indicates degraded AI response format consistency and should prompt investigation of recent Claude model behavior or prompt changes; a `"critical"` status indicates the problem is widespread and confidence fields are routinely missing. **Interpreting `rateLimitCleanup.status: "warn"`**: the periodic cleanup of the `rate_limit_log` table has errored at least once; check server logs for DB advisory-lock or delete failures — stale rows will accumulate if cleanup stays broken.
