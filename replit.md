...
- **Metrics API**: `GET /api/metrics` returns in-memory runtime counters. Currently exposes `aiFixRetry.count` (how many times `fixComplianceIssue` fell back to the strict-prompt retry path since last server restart) and `aiFixRetry.lastAt` (ISO timestamp of the most recent retry). Also exposes `sessionSaveFail.count` (how many times `session.save()` failed after a token refresh since last server restart) and `sessionSaveFail.lastAt` (ISO timestamp of the most recent failure); a non-zero or rising count indicates the PostgreSQL session store is degraded and users are receiving in-memory-only sessions. No auth required. All counters reset on server restart.
