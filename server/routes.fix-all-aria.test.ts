...
// ---------------------------------------------------------------------------
const { mockFixAllAriaRoleMisuse, mockDbSelectWhere, mockDbUpdateReturning } = vi.hoisted(() => ({
  mockFixAllAriaRoleMisuse: vi.fn(),
...
// ---------------------------------------------------------------------------
// Mock: accessibility-engine – exposes fixAllAriaRoleMisuse as a spy so
...
// ---------------------------------------------------------------------------
// Mock: markdownTableConverter – not exercised by fix-all-aria; passthrough.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Mock: table-fixers – not exercised by fix-all-aria; passthrough.
// ---------------------------------------------------------------------------
...
//   status === "completed" && accessibleHtml !== null && complianceReport !== null
...
//
// A minimal req.session shim is required because the fix-all-aria handler
...
describe("POST /api/conversions/:id/fix-all-aria — wasRetried forwarding", () => {
  ...
  it("includes wasRetried: true in the JSON response when the fixer returns wasRetried: true", async () => {
    // Simulate the retry path: fixAllAriaRoleMisuse explicitly signals that
    // its first AI attempt failed and a second attempt was needed.
    mockFixAllAriaRoleMisuse.mockReturnValue({
      accessibleHtml: ACCESSIBLE_HTML,
      complianceReport: COMPLIANCE_REPORT,
      wasRetried: true,
    });
    ...
    const response = await request(app)
      .post("/api/conversions/1/fix-all-aria")
      .expect(200);
  ...
  it("includes wasRetried: false in the JSON response when the fixer does not set the flag", async () => {
    // Simulate the happy path: fixAllAriaRoleMisuse returns without
    // wasRetried, so the route's `?? false` fallback must supply false.
    mockFixAllAriaRoleMisuse.mockReturnValue({
      accessibleHtml: ACCESSIBLE_HTML,
      complianceReport: COMPLIANCE_REPORT,
