...
export const ACCESSIBILITY_ENGINE_EXPORTS_USED_BY_ROUTES = [
  ...
  "getFirstHeadingLevel",
  "buildHeadingRenumberedNoteHtml",
  "buildMainLandmarkNoteHtml",
  "buildPageTitleFallbackNoteHtml",
...
export function createAccessibilityEngineMock(
  ...
  return {
    ...
    getContextLeakMetrics: vi.fn().mockReturnValue({ count: 0, lastAt: null }),
    applyHeadingHierarchyFix: (html: string) => html,
    applyCustomPageTitle: (html: string) => html,
    getFirstHeadingLevel: () => null,
    buildHeadingRenumberedNoteHtml: () => "",
    buildMainLandmarkNoteHtml: () => "",
    buildPageTitleFallbackNoteHtml: () => "",
    buildPageTitleLowQualityNoteHtml: () => "",
