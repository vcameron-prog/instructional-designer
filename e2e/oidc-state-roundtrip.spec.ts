/**
 * Smoke test: signed OIDC state survives the round-trip to /api/callback.
 *
 * WHY THIS TEST EXISTS
 * --------------------
 * The unit / integration tests in returnTo.test.ts mock passport.authenticate,
 * so they never exercise the actual openid-client/passport Strategy's state-echo
 * behaviour.  The core assumption — that Replit's OIDC provider echoes back our
 * custom `state` parameter in the callback URL — is only observable by going
 * through the real authorization-URL generation path.
 *
 ...
 *
 *   Step 2 — GET /api/callback?state=<captured-state>&_test_state_only=1
 *     We replay the captured state through the REAL /api/callback endpoint
 *     using a dev/test bypass (active only when PLAYWRIGHT_TEST=1 and
 *     NODE_ENV !== "production").  The bypass skips the OIDC token exchange
 *     (which requires an interactive browser login) but runs the same
 *     verifyReturnToState() + redirect logic as the production code path.
 ...
 * The _test_state_only bypass in /api/callback is registered only when
 * PLAYWRIGHT_TEST=1 AND NODE_ENV !== "production" (see replitAuth.ts).
 *
 * Run with:
 *   npx playwright test e2e/oidc-state-roundtrip.spec.ts
 */
...
async function captureStateFromLoginRoute(
  ...
  try {
    await page.goto(
      `${ROOT}/api/login?returnTo=${encodeURIComponent(returnTo)}`,
...
test.describe("OIDC signed-state round-trip smoke tests", () => {
  test(
  ...
  test(
    "signed state from the OIDC authorization URL lands on the correct page after /api/callback",
    async ({ page }) => {
      ...
      // Replay the captured state through the REAL /api/callback endpoint
      // using the dev/test bypass (_test_state_only=1).  This exercises the
      // same verifyReturnToState() + redirect logic that production uses after
      // passport.authenticate() succeeds — without requiring a live OIDC
      // token exchange.  A real Replit OIDC provider echoes the state param
      // back in this exact way.
      await page.goto(
        `${ROOT}/api/callback?state=${encodeURIComponent(capturedState!)}&_test_state_only=1`,
      ...
      await expect(
        page,
        "browser must land on the returnTo destination when /api/callback receives the echoed state"
      ).toHaveURL(/\/faculty/, { timeout: 10_000 });
    }
  );

  test(
    "/api/callback with no state param falls back to the root page",
    async ({ page }) => {
  ...
  test(
    "/api/callback with a tampered state param falls back to the root page",
    async ({ page }) => {
  ...
  test(
    ...
    async ({ page }) => {
      ...
      try {
        await page.goto(`${ROOT}/api/login`, {
          waitUntil: "commit",
          timeout: 15_000,
        });
      } catch {}
